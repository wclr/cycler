...
test.only('Basic driver - `filter` and `select` methods', (t) => {
  const johnSymbol = Symbol()
  const requests: Request[] = [
    { name: 'John', category: johnSymbol },
    { name: 'Alex', type: 'alex' },
  ]
  ...
  source
    // this won't work
    .select<{ addToResponse: boolean }, { addToRequest: boolean }>(johnSymbol)
    .map((r$) => {
      t.is(
